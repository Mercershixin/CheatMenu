...
> **4. ⛔ 不做仿真 / 循环测试（用户三次强调，2026-09-19 定稿）。**
> - **默认命令就是 `python .workbuddy/build/push_now.py --fast`** —— `--fast` 会**跳过整脚本仿真与循环点击测试**。
> - **不要**主动跑 `verify_all.py`、`local_sync.py --full`、`sim/run_full.py`、`run_probe_loader.py`，
>   也**不要**为了"验证一下"临时跑仿真 —— 这些**只在用户明确说"跑一下/验证一下/查一下"时才做**。
> - 改完一个功能 = **钉版本号 → `push_now.py --fast`（+ `push_docs.py` 若动了 README/事件库）**，就结束。
> - 如果某次确实需要全量门禁（例如用户要求），再显式跑一次，并在回复里说明是应要求跑的。
...
> ⚠️ 本地工作区里是**完整源码，带注释**（**文件名跟版本号走**：`CheatMenu-<版本>.lua`，如 `CheatMenu-6.8.1.lua`，
