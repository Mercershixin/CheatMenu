...
### 9.1 五个只读审计脚本（`.workbuddy/build/`，随时可重跑复核）
...
| `_audit_errcheck.py` | **错误体检（v9.9.1 新增）**：重复表键 / 重复函数定义 / 冒号点号用混 / `string.format` 占位符数 / 乱码·替换符 / 同页重复 UI 标签 —— 覆盖"函数·逻辑·符号·文本"四类错 |
| `_audit_luapitfalls.py` | **Lua 常见坑（v9.9.1 新增）**：写全局 / `table.sort` 非严格弱序 / `%d` 配小数 / 遍历中删同一张表 / 延迟回调陈旧捕获 / `os.time()` 计时 |
...
- **自己写的 mask（把注释/字符串体抹成空格保留偏移）不能用来【读内容】**：用它提取键名时
  `["Noobini"]` 会变成 `["       "]`，所有键退化成同一个空串 ⇒ 第一版凭空报出几百个"重复表键"。
  **提取键名/字面量必须按偏移回原文取**（偏移是对齐的，可以用来定位；内容要回原文读）。
- ★ **`local` 声明晚于使用点不会报错，只会静默写全局** —— v9.9.1 的 `T2O_N` 就是这么来的
  （清缓存写入的是全局，真计数器没归零；同时泄漏一个可被检测的名字，违反 §2 中性化口径）。
  日常门禁 `check_globals.py` **查不出来**（它只查 `Unknown global`，这是写不是读），
  要靠 `luau-analyze` 的 **`GlobalUsedAsLocal`** 那类诊断。**看到这条别当成 lint 噪音。**
- ★ **写"Lua 常见坑"探测器时，这三类形态是【正确代码】，必须排除否则满屏假报**
  （`_audit_luapitfalls.py` 第一版就是这么翻车的）：
  ① **表构造里的字段键** `AntiAFK=true,` 不是全局写 —— 要按"逗号切分 + 只抠 `Name=` 那一小段"排除，
     且**只排除键本身**（`Callback=function() ... end` 的函数体照样要扫）；
  ② **`table.remove(t,i) break` 是安全惯用法** —— 删一个立刻跳出，不会再迭代，没有下标塌陷；
  ③ **upvalue 被循环重新赋值、闭包引用它** —— 这是 Roblox 里【刻意的】写法
