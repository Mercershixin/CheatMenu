...
| ③ | **真云端分发链路验证** | 真网络 / 真 CDN / 真字节：跑 `cloud_verify.py`，验**云端实际发给用户的字节**是否与本地发行产物逐字节一致、能否编译。**不需要真机，也不需要在执行器里跑 Lua 功能**（用户 2026-09-20 定稿） |
...
> ★★ **2026-09-20 追加（用户原话：「也包括循环检测 也禁用，只能用我说3个测试的 那个」）**
>
> ⛔ **「循环检测」一并【彻底禁用】—— 而且【没有例外】。**
> 它指的就是文档旧写的「**仿真 / 模拟检测循环 / 多轮深度验证**」，
> 也就是「**模拟 → 修复 → 再模拟 → 再修复**」那种反复跑的检测循环
> （旧 README / 旧 CHANGELOG 里"**只在用户明确要求时才做**"的口子 —— **已作废，不许再援引**）。
>
> **本项目只认 ①②③ 三种手段。** 仿真台、模拟检测循环、多轮深度验证、任何形式的新 mock 门禁 —— 一律禁跑。
> 用户要真的验证，就走 **③ 真云端分发链路验证**（`cloud_verify.py`：真拉、比字节、把云端字节送编译器），
> **不要**在本机拿 mock 代替。
>
> ★★ **2026-09-20 追加（用户原话：「云端不需要发到实机和在 lua 使用功能，你就纯分发的真云端就行」）**
> 第三类**不再要求真机、也不需要在执行器里跑 Lua 功能**；原 v7.8.0 的 `SYS.CloudCheck()` 执行器自检
> **已按该要求删除，不要再加回来**。
...
1. **不要自作主张跑「测试循环 / 多轮深度验证 / 循环检测」。**
   ...
   **只在用户明确说「跑一下 / 验证一下 / 测试一下 / 发版」时才跑**。
   ...
   ① 本节旧版"发版必须跑仿真门禁"；② README 旧版"仿真 / 模拟检测循环只在用户明确要求时才做"。
   ⛔ **整脚本仿真 与「模拟检测循环 / 多轮深度验证」（= 循环检测）已【一并彻底禁用】，没有任何例外** ——
   不是"用户要求就能跑"，而是**本项目只认 ①②③ 三种手段**（见 §0.1）。
   现在的发版门禁 = 等价性 + `luau-compile` + 加载器探针，`publish.py` / `push_now.py` **不带 `--fast` 也能过**。
   ⛔ 不要"顺手跑一次仿真"，不要"修一下仿真台让门禁变绿"，更不要跑"模拟→修复→再模拟"的检测循环。
...
> **同日追加：「云端不需要发到实机和在 lua 使用功能，你就纯分发的真云端就行」** —— 第三类已细化为「**真云端分发链路验证**」(`cloud_verify.py`)。
> **追加原话：「也包括循环检测 也禁用，只能用我说3个测试的 那个」**
>
> **不要运行、不要修复、不要扩展。** 只允许的三种验证手段见 **§0.1**。
> ⛔ 「模拟检测循环 / 多轮深度验证」（循环检测）同样**彻底禁用、无例外**。
