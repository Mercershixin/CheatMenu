...
**用户明确说"还需要"的，别再删**：`NoClip`（穿墙）· 无限跳 · Ctrl+数字路径点。

> ⚠ **`AntiFling`（防甩飞）已于 12.1.6 删除** —— 它原本也在这条清单里，但**界面入口在 11.8.5 撤掉、
> 且 `AntiFling` 键从未在 `T_` 表里声明**（连启动恢复都带不动它）⇒ 全项目零引用、从来用不到。
> 用户 2026-09-25 明确拍板「直接删掉」（连同 `FLING_MAX` / `SYS._flingN`）。
> ★ 教训：**保护清单要跟着现实走** —— 一条"用户说还需要"的功能，如果界面和配置键都撤了，
> 它就从"待恢复"变成"死代码"；光靠注释挡不住清理，得逐条核实现状。想要它回来 = 重新实现。
...
| **过检测**（防回退） | **上报给服务端多快** | 格/秒（`C_.RevertCap`，**0 = 只拦瞬移**；`C_.RevertJump=150` 单帧大跳阈值） | `T_.AntiRevert`（**默认开**） |

★ **防回退默认开为什么无害**：正常跑/飞每帧位移 ≤ ~10 格，而 `RevertJump` 阈值为 **150 格/帧** ⇒ **只对 TP 那种单帧几百格的瞬移生效**，对正常移动零影响。
⚠ 但**不能靠它把持续高速伪装掉** —— 一旦把 `RevertCap` 调到**低于实际速度**，服务端就会把你拽回（= 定在原地）。
⇒ **「跑得快」与「看起来正常」不可兼得**，这是物理限制，不是参数问题。
...
- ★ **发版会自动跑「③ 开关四点对账」**（2026-09-24 接入 `push_now.py`）：输出
