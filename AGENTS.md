...
（8.x 核验：上表这些键在当前源码里**只出现在 `REMOVED_FEATURES` 黑名单中**，后端确实没有了。）
...
| 命中率 / 漏打模式 | `CB_MissMode` 等残键仍在 |
| 快照瞄准 | `CB_SnapFire` 等残键仍在 |
| 静默瞄准 | `CB_Silent` 恒 `false`，`CB_SilentAim` / `CB_SilentNoTurn` 残键仍在 |
...
> ⚠ **残键的处置口径：不要顺手删。** 战斗主循环仍在读它们，删键要先改主循环，**风险另评**。
...
| 分节 | 行数 | 说明 |
|---|---|---|
| `[00]` 全局状态 | 198 | `SYS` 表 + 全部开关/参数初值 |
| `[00B]` 反指纹 | 45 | 中性名表 + GENV 键别名 |
| `[01]` 服务 + 工具 | 56 | `T` / `TT` / `P` / `DS` 等 |
| `[02]` 配置持久化 | 279 | `SYS.C_` / `SYS.T_`、`SaveConfig`、`SaveRemotes` + **`SaveDump`** |
| `[03]` 角色 / 相机 / 控制 | 147 | |
| `[04]` 远程事件 | 467 | `RemoteAlias` / `FindEvent` / `ProbeEvent` |
| `[05]` 移动模块 | 740 | 飞行 / 跳跃 / 路径点 / 手动选服 |
| `[06]` 上帝 / 全亮 / 玩家穿透 | 1576 | |
| `[07]` 帧率优化 | 290 | 含纹理 / 画质降级 |
| `[08]` 防踢 / ESP / 自由视角 | 1603 | **`SYS.Index()` 统一扫描索引器** + `SYS.Kw*` 共享判据表 |
| `[08.5]` 子弹射线 | 143 | 纯本地可视化 |
| `[09]` 基地操作 | 498 | 收脑红 / 收金币 / 低 CPS 售卖 |
| `[10]` HUD / 训练 / 重生 / 健身房 | 438 | |
| `[11]` 传送 | 134 | |
| `[11.5]` **战斗模块** | 1902 | **全文件最大单体**；含 `@@BALLISTIC_BEGIN/END` 弹道段（**禁改**） |
| `[12]` **翻译模块 v120.2** | 2026<br>（L9163–L11188 段行数） | 11 层重写；**只走本机 llama.cpp**（云端后端已删，见 §2 A）。v120.2 新增：富文本(带标签)保护路径 + 槽位预热 + 自适应并发。**读代码前先看模块头的分层说明** |
| `[13]` UI 工具 | 458 | 玻璃拟态 + 霓虹渐变（**外观红线区**） |
| `[13.5]` 游戏内通知 | 41 | 自实现通知条，不依赖任何 UI 库 |
| `[12B]` ChronixHub 融合 | 1696 | 逻辑层，全部可逆，只写本客户端 |
| `[12C]` 反作弊对抗靶场 | 557 | `SYS.AC` |
| `[14]` UI 页面 | 831 | **外观红线区** |
| `[14.6]` 实验室 | 1457 | 执行器高级 API 实验区 + **综合扫描（十层）** |
| `[14.9]` 新增页面 | 101 | 玩家(动手类) / 整蛊工具 |
| `[15]` 创建菜单 | 1018 | **外观红线区**：硬编码 `W,H=830,664`，上千控件全按**绝对偏移**画 |
| `[16.1]` 翻译忽略名单 | 354 | ★**用户维护的数据**，不是逻辑 —— **别当代码重构** |

### 8.2 结构指标（实测）

- **main chunk 的「每函数 200 个 local」名额：76 / 200**（其中 `local function` 40 个，主要来自**未隔离**的 `[11.5]`）
  - ★ Lua/Luau 的 local 上限是**每函数 200 个同时活跃的**；`do..end` 块结束时局部**会释放**。
    所以 **大模块用 `do..end` 包起来**就不吃 chunk 名额（`[12]` 翻译模块就是这么重写的）。
  - ⛔ **别再用"数行首无缩进的 `local`"来估算** —— 会严重高估（那样数出来是 202，实际 76）。
    用 `_audit_chunklocals.py`（逐 token 扫描；**自检：收尾块深度必须恰为 0**）。
- **62 个 `SYS._*` 共享态表 / 211 处引用** —— 模块私有状态被摊在全局命名空间里。
...
- **配置键对账**：`SYS.C_` 声明 **81** 键 / 代码引用 **83** 键，
...
- **函数级重复：0 组**（625 个函数定义参与归一化 sha1 比对）—— 无复制粘贴屎山。
- **已弃用 API：`FindPartOnRay` ×16 + `FindPartOnRayWithIgnoreList` ×1**（新老混用：`WS:Raycast` ×3）。
  → 迁移是**等价性敏感**的（返回形态 / 忽略语义 / `IgnoreWater` / `CollisionGroup` 默认值都不同），
...
| ② | **状态层归位**：62 个 `SYS._*` → `ESP._npcList` 等模块表 | 低 | 全是**内部状态、无对外契约** → 不涉及"对外成员必须 100% 保留"的约束；从结构上消灭作用域坑 |
| ③ | **`[11.5]` 战斗模块分层** + 裸块隔离 | 中 | 7 层（数据 / 工具 / 目标 / **弹道(禁改)** / 动作 / 驱动 / 诊断）；**绕开残键清理**（§2-B）；**弹道段改必跑 `_ballistic_unit.py`** |
| ④ | **弃用 API 迁移**（`FindPartOnRay` ×16） | 中 | 逐站点等价核对，**禁批量替换** |
