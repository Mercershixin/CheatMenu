...
UI.Pages["设置"]=function(p)
    ...
    UI.Div(p)
    UI.Switch(p,"🫥 深度隐藏菜单 (挂执行器隐藏容器, 位置可能偏)",function() return SYS.C_.SafeGui end,function(v)
        SYS.C_.SafeGui=v and true or false
        SYS.Notify("下次打开菜单生效(重新加载脚本最稳)",SYS.CY.yellow)
    end)
    UI.Tip(p,"开着=菜单挂到执行器的隐藏容器(游戏本地反作弊更难扫到), 但【有些执行器那个容器不是全屏】-> 菜单可能跑到角落或被压小。\n默认关闭 = 挂 CoreGui(全屏、位置正常, 也比 PlayerGui 隐蔽)。手机版建议保持关闭。",CY.sub)
    UI.Div(p)
...
function SYS.SafeParentGui(gui)
    ...
    -- ★ v4.6.0 修「手机上菜单挤在左上角、还缩得很小」(用户实测截图):
    --   原来【优先】挂 gethui() —— 但有些执行器的 gethui 返回的容器【不是全屏】(尺寸未知/很小),
    --   而菜单的位置是 `0.5*父容器宽 - W/2` 这么算的 -> 父容器一错, 位置就全错、还被压小。
    --   现在默认挂 CoreGui(全屏 / 比 PlayerGui 隐蔽), gethui 改成【可选】的深度隐藏。
