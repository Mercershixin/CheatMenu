...
local function alive(pl)
    ...
    if h then
        ...
        -- ★ v68: Health 归零可能延迟, GetState() 更及时 —— 双保险判死, 避免"锁在死人身上"
        local ok,st=pcall(function() return h:GetState() end)
        if ok and st==Enum.HumanoidStateType.Dead then return false end
        return true
    end
    -- ★ v76 权威死亡信号(只读监听得来, 不靠猜):
    --   游戏自己报过他死了 -> 直接判死; 超过 TTL 就当作"信号过期"自愈(防止没收到复活事件时把人永久判死)
    local dt=CB.DeadAt[pl.Name]
    if dt then
        if (os.clock()-dt)<(CB.DeadTTL or 8) then return false end
        CB.DeadAt[pl.Name]=nil
    end
    -- ★ v76 游戏自己的敌人高亮容器里有他 -> 游戏认为这是"在场的敌人", 放行
    --   (很多游戏死后销毁 Humanoid 只留尸体, 原来会因此索不到人; 有这层旁证就稳了)
    if CB.GameSaysEnemy and CB.GameSaysEnemy(pl) then return true end
    -- 没有 Humanoid 的游戏(纯自定义角色): 只要有实体躯干就当作还活着
    -- ★ v71 修「还会锁定死了的人的身体」: 很多游戏死亡后会【销毁 Humanoid、只留下尸体模型】,
    --   上面这条兜底于是把尸体判成"活着" —— 这正是锁死人的根因。
    --   开启「只锁活人」(默认) 后, 没有 Humanoid 的角色一律不算活人。
