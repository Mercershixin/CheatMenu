...
local SYS={
    ...
    T_={
        ...
        RemoteSpy=false,   -- ★ v69.4 RemoteSpy 抓包(默认关)
        ...
        CB_OnlyAlive=true,
        CB_SkipFF=true,        -- ★ v106 无敌盾: 跳过带 ForceField 的目标(等护盾结束再打)
        -- ★ v122 反陷阱免伤(地图道具/生物/陷阱的攻击与控制; 实现见 [05] 移动模块)
        TrapImmune=false,
        -- ★ v71 透视方框(附加项; 透视主体是人物高亮 Highlight)
        ESPBox=false,
...
do
    ...
    -- ★ v102 移植(参考 1.txt 的 SaveManager): 多套【命名配置】—— 存档/读档/删档/列表。
    --   与上面的自动配置互不影响: 自动配置始终存在(CFG), 这里是用户手动存的"档"。
    local PROF_PREFIX="CheatMenu_Profile_"
    local function profFile(name)
        local s=tostring(name or ""):gsub("[%c%s]","")   -- 去掉控制符/空白(保留中文/字母数字)
    ...
    function SYS.SaveProfile(name)
        ...
        -- ★ v105 修: 连开关状态(T_)一起存 —— 用户存"战斗档"时期望加载回来开关也还原。
        --   (自动配置 CFG 仍然不存 T_, 保持"每次启动默认全关"的既定行为; 只有手动存档才存。)
        local ok=P(function()
            local d=profCollect()
            d.__T_={}
    ...
    function SYS.LoadProfile(name)
        ...
        if ok then
            ...
            -- ★ v105: 先把所有页面构建出来, 保证每个开关的 onChange 都已登记(SwitchOnChange)
            if SYS.EnsurePage then
                for _,def in ipairs(UI.Defs or {}) do P(SYS.EnsurePage,def.name) end
            end
            -- ★ v105: 真正把开关应用起来(复用脚本启动时那套逻辑): 先统一关, 再按 T_ 开,
            --   避免"上一份档开着、这份档关着"的功能残留。
            ...
            for _,fn in ipairs(SYS.BtnRefs or {}) do P(fn) end     -- 刷新 UI 显示
...
--============================================================
-- [04.5] RemoteSpy · 记录全部 RemoteEvent / RemoteFunction 的收发
--   为什么需要它: 静态扫描(SYS.DumpRemotes)只能列出"场景里有哪些 remote";
--   要判断"这个 remote 到底发了什么、有哪些字段可以汉化", 必须真抓到流量。
--   做法与 SimpleSpy 同源(不是抄它的界面, 是同一套捕获点):
--     ① 发出: hookmetamethod(game,"__namecall") -> FireServer / InvokeServer
--     ② 收到: 给每个 RemoteEvent 的 OnClientEvent 加一个记录器(与游戏的监听并存, 不改行为)
--     ③ 收到(RemoteFunction): 包一层 OnClientInvoke, 记录后原样调用原回调(不改行为)
--   新出现的 remote 由 RStorage.DescendantAdded + 5 秒兜底扫描补挂。
--   ⚠ 默认关。全局 hook __namecall 影响所有方法调用, 所以 hook 体必须极短(先判 Active 直接透传);
--     并且 namecall 的 hook 装上就【卸不掉】(执行器限制) —— 停止后只是不再记录, 每次调用多一次判空。
--   ⚠ 它只"看", 不伪造。要发假包用 SYS.Fire / SYS.RFunction。
--============================================================
do
    ...
    -- 把参数压成短而可读的字符串(过长截断, 表格只报项数, 实例报 类名+名字)
    ...
    -- 入缓冲 + 维护"每个 remote 的唯一参数签名"(导出汇总就是靠它)
    function Spy.push(dir,name,args)
        if not Spy.Active then return end
        local now=os.clock()
        if now-Spy.RL<0.02 then return end        -- 限流: 最多 50 条/秒, 防刷屏/防卡
        Spy.RL=now
    ...
    -- 导出汇总: 按 remote 归类 -> 每个 remote 出现过哪些参数组合(可直接拿去做字段汉化映射)
    ...
    local function hookIncoming(o)
        if o:IsA("RemoteEvent") then
            ...
            local ok,c=pcall(function()
                return o.OnClientEvent:Connect(function(...)
                    if not Spy.Active then return end
                    -- ★ 注意: 下面这个匿名函数【不是】变参函数, 里面不能直接用 ... —— 先打包成局部变量
                    local a=table.pack(...)
        ...
        elseif o:IsA("RemoteFunction") then
            if Spy.RFWrapped[o] then return end        -- 包装不可逆, 只包一次
            pcall(function()
                local old=o.OnClientInvoke
                if type(old)=="function" then
                    Spy.RFWrapped[o]=true
                    o.OnClientInvoke=function(...)
                        local a=table.pack(...)   -- ★ 同理: 不能在内层匿名函数里用 ...
                        if Spy.Active then
            ...
            if type(hookmetamethod)=="function" and type(getnamecallmethod)=="function" then
                pcall(function()
                    ...
                    orig=hookmetamethod(game,"__namecall",function(self,...)
                        -- ★ 极短快路径: 没开就立刻透传(全局 hook, 每秒可能被调上千次)
                    ...
                    -- ★ 用 hookmetamethod 的【返回值】当原始函数(见战斗模块那段注释, 否则自递归爆栈)
                    if type(orig)=="function" then Spy.HookOK=true end
                end)
            end
            -- ② 收到: 先全扫一遍, 再挂新对象钩子 + 5s 兜底
...
do
    ...
    function SYS.FlyTick(dt)
        ...
        -- 模式 Align(默认): Attachment + LinearVelocity + AlignOrientation
                ...
                if hit and (os.clock()-hit)<TRAP_WIN then
                    ...
                    if a then
                        ...
                        if hzm>TRAP_BACK then
                            local dir=hz.Unit
                            local np=hrp.Position-dir*math.min(hzm,TRAP_MAXSTEP)   -- 渐进回拉
...
do
    ...
    -- ★ v68.6 隐身(参考 Infinite Yield invisible + CJ_Invisible)
    --   ⚠ 这是【本地隐身】: 只改 Transparency/LocalTransparencyModifier/DisplayDistanceType,
    --   都是客户端渲染属性 —— 自己屏幕上看不到自己, 但服务器和其他玩家看到的你【完全正常】。
    --   要"真隐身"必须服务器改你的角色(或游戏有隐身道具可伪造), 客户端通用脚本做不到。
    local InvisConn
    -- ★ v68.8 备份原始透明度: 有些部件【本来就隐藏】(游戏辅助碰撞盒/特效), 关闭时
    --   无脑恢复 0 会把它们显形(实测: 关闭后身上出现方块状物体)。弱表防泄漏。
        ...
        if on then
            ...
            DeepHideConn=RS.RenderStepped:Connect(function(dt)
                ...
                if r and DH_ACC>=1/30 then
                    ...
                    --   被打断。现在只有【沉太深】(会被引擎销毁)才硬拉回; 被服务器往上拉时改用
            ...
            if root then
                ...
                -- ★ v87: 渐进浮回 —— 旧写法一次 CFrame 从地下拉回地表 = 单帧几百 studs 瞬移,
                --   服务端判 Illegal Teleport(错误 267 踢出, 用户实测)。改成小步上浮:
                --   每步 8 studs / 0.1s ≈ 80 studs/s(与跳跃/被击飞同量级), 300 格约 4 秒浮完。
                --   只在角色被销毁时中止; 卸载(UnloadAll)触发的浮回也要浮完, 不看 Unloaded。
                task.spawn(function()
...
do
    ...
    -- ★ v69: ESP 的绘制容器(懒取一次)。优先 gethui()(和主菜单同一条路, 一定画得出来),
    --   退到 CoreGui / PlayerGui。旧实现把 Highlight 挂在 Workspace 上, 见下面 ESPTick 注释。
...
local function alive(pl)
    ...
    -- ★ v100 修「人被打死倒地了还在锁、还在开火」:
    --   游戏权威死亡信号(DeadAt)原来【只在没有 Humanoid 时才看】, 但有些游戏死后 Humanoid 仍在
    --   (Health 不归零/延迟归零) + 角色 Ragdoll 倒地 -> 被当成活人继续锁+开火。
    --   现在把 DeadAt 提到最前: 游戏报过他死了, 一律判死, 不看 Humanoid 在不在。
    local dt=CB.DeadAt[pl.Name]
    if dt then
        if (os.clock()-dt)<(CB.DeadTTL or 8) then return false end
        CB.DeadAt[pl.Name]=nil   -- TTL 过期 -> 信号失效(自愈), 防止收不到复活事件时永久判死
    end
    local h=humOf(pl)
    if h then
        if h.Health<=0 then return false end
        -- Health 归零可能延迟, GetState() 更及时; Dead 与 Physics(布娃娃倒地) 都算死
        ...
        -- PlatformStand(平台站立)=常见的倒地/被控表现; 只在「只锁活人」开时才据此判死(降低误伤)
        if SYS.T_.CB_OnlyAlive and h.PlatformStand==true then return false end
        return true
    end
    -- ★ v100 「只锁活人」(默认开): 没有 Humanoid = 尸体, 一律不算活人。
    --   必须放在 GameSaysEnemy 之前 —— 游戏的高亮容器是延迟清理的, 原来会把尸体放行(锁死人根因之二)。
    if SYS.T_.CB_OnlyAlive then return false end
    -- 只有关了「只锁活人」才用游戏的旁证兜底(纯自定义角色、没有 Humanoid 的游戏)
...
local function clearShot(part)
    ...
    -- ★ P() 返回的是 pcall 的【多个】返回值: 第一个是成功与否的 boolean,
    --   结果在第 2 个。只取一个变量会拿到 true, 后面 hit:IsDescendantOf 直接报错。
    -- ★ v63: 这是脚本自己的隔墙检测, 用 inOwnRay 标记, 免得被工作区射线 hook 替换成假射线
    --   (否则"隔墙不打"会被静默的 fakeRay 干扰, 永远判定"无障碍")。
    -- ★ v67: inOwnRay 复位放在 P() 调用之外 —— 射线抛错时标志也不会卡死在 true。
    CB.inOwnRay=true
...
--======================== 指定目标 (v53 新增) ========================
--  两种选人方式:
--    · 自动 —— 按 CB_Priority(最近 / 血最少 / 正在瞄我 / 最靠中)自己挑
...
--   ★ v111(用户 2026-09-17 要求): 默认链改为「最近 -> 正在瞄我的 -> 屏幕中心」。
--   注意「指定目标」不在链里 —— 它在 pickTarget 最前面单独判定, 永远压过整条链,
--   所以实际生效顺序 = ①我指定的目标 ②最近的敌人 ③正在瞄我的 ④屏幕中心。
--   无敌盾的人直接不进候选池(CB_SkipFF), 等护盾结束再恢复。
local CB_CHAINS={
    [1]={"near","aiming","center"},   -- 最近 -> 攻击我的 -> 屏幕中心 (默认)
    ...
    for i=1,#chain do
        ...
        if key~="crosshair" then
            ...
            for j=1,#cands do
                local c=cands[j]
                if key=="aiming" then
                    -- 只有【正在瞄我】的人才算命中这一环; 没人在瞄我就整环为空 -> 落到"最近"
            ...
            if #pass>0 then
                -- aiming 环内用 near 作次优键(多个在瞄我的人里选最近的)
...
local function crosshairOnEnemy()
    ...
    -- ★ v63: 准星检测是脚本自己的射线, 标记 inOwnRay 避免被工作区 hook 替换。
    -- ★ v67: inOwnRay 复位放在 P() 调用之外 —— 射线抛错时标志也不会卡死在 true。
    CB.inOwnRay=true
...
function CB.QuickMode()
    ...
    SYS.C_.CB_PrioMode=1   -- ★ v111: 一键开战默认优先链 = 最近 -> 攻击我的 -> 屏幕中心(指定目标仍最优先)
...
-- ★ v85: 玩家名清单 —— 顶栏/记分板/头顶名牌这类整条就是玩家名的东西, 不翻
...
-- ★ v85: 官方顶栏(Roblox 顶栏/游戏顶栏按钮: 倾斜/对话气泡/麦克风/闪电/三点...)
--   名字带 "topbar" 的祖先一律不碰 —— 用户要求官方 UI 保持原样
...
local function CreateMenu()
    ...
    local function ApplyScale()
        P(function()
            ...
            --   旧逻辑 hi=1.25(小屏还放大) + lo=0.75(下限抬高) —— 两条都会让
            --   【菜单比屏幕还宽】, 于是右侧/底部的按钮永远点不到(用户实测反馈)。
            --   现在: 上限恒为 1(只缩不放, 保证一定塞得进屏幕), 下限降到 0.30 只在极端小屏兜底。
            local hi=1
            local lo=0.30
...
function SYS.ToggleMenu()
    ...
    if SYS.MenuOpen then
        ...
        if SYS.FreeCamActive then
            ...
            -- ★ v101 恢复"打开菜单前"的鼠标状态; 只有没记录过时才回退到脚本加载时的值。
            --   旧实现直接用脚本加载时的值(常常是 Default) -> 关掉菜单后鼠标一直可见。
...
-- ★ v68: 一键紧急隐藏 (参考 GitHub ADMIN-HUB PanicHide 补齐)
--   紧急时快速隐藏所有作弊痕迹: 菜单 + 透视 + 名字标签 + 战斗叠加层(FOV圈/高亮/距离) + 自由视角。
--   不改变任何开关状态; 按 G 键重新打开菜单即可, 战斗需重新开关一次才恢复瞄准。
