...
function LAB.FullScan()
    ...
    print("")
    print("####################  🔬 综合扫描  ####################")
    print(("时间 %s   执行器能力: getgc=%s hook=%s getscripts=%s")
        :format(os.date("%H:%M:%S"),tostring(LAB.Caps().getgc),tostring(LAB.Caps().hookfn),tostring(LAB.Caps().scripts)))
    print("")

    -- ---------- A 通信层: 复用设置页那个 DumpRemotes(它自己会打完整清单, 并返回列表) ----------
    print("════════════ A · 通信层(游戏接口: 你能触发什么) ════════════")
    local remotes
    local okR=P(function() remotes=SYS.DumpRemotes() end)
    if not okR then print("  [!] Remote 扫描失败") remotes=nil end
    -- ★ 补细节: 按"服务名"归组 + 关键词高亮(战斗/经济/社交/角色), 只挑有用的打出来
    if type(remotes)=="table" then
        local GROUPS={
            {name="⚔ 战斗/伤害", kw={"Combat","Damage","Hit","Killed","Death","Die","Weapon","Ammo","Reload","Equip"}},
            {name="💰 经济/商城", kw={"Shop","Buy","Purchase","Product","Gamepass","Gacha","Spin","Reward","Claim","Coin","Cash","Sell"}},
            {name="📦 物品/背包", kw={"Inventory","Item","Stack","Backpack","Loot","Pickup","Collect","Drop"}},
            {name="👥 社交/交易", kw={"Trade","Friend","Postie","Social","Vote","Chat"}},
            {name="🏃 角色/移动", kw={"Character","Teleport","Respawn","Spawn","Jump","Move","AFK","Suicide"}},
        }
        local gen=0
        for _,g in ipairs(GROUPS) do
            local hits={}
            for _,r in ipairs(remotes) do
                for _,k in ipairs(g.kw) do
                    if r:find(k,1,true) then hits[#hits+1]=r break end
                end
            end
            if #hits>0 then
                print(("  ── %s (%d 个) ──"):format(g.name,#hits))
                for i=1,math.min(#hits,14) do print("     "..hits[i]) end
                if #hits>14 then print(("     ... 还有 %d 个"):format(#hits-14)) end
                gen=gen+#hits
            end
        end
        print(("  (命中可做功能的关键词: %d 个; 完整 %d 条见上面的 Remote 清单)"):format(gen,#remotes))
    end
    print("")

    -- ---------- B 代码层 ----------
    print("════════════ B · 代码层(游戏函数: 它是怎么实现的) ════════════")
    LAB.ScanGC()          -- 函数/表 + 按归属脚本
    LAB.ListHookable()    -- 可疑函数(名字含 damage/fire/aim…)
    print("")

    -- ---------- C 脚本层 ----------
    print("════════════ C · 脚本层(游戏跑了哪些脚本/模块) ════════════")
    LAB.ListScripts()
    print("")
    print(("####################  扫描完毕 (%.2fs)  ####################"):format(os.clock()-t0))
    SYS.Notify("综合扫描完成 —— 全部结果在控制台(F9)",SYS.CY.green)
end

--  ══════════════════════════════════════════════════════════════
--  ② 一键抄走: 把本次扫描的关键结论按"可粘贴"格式复制到剪贴板
--  ══════════════════════════════════════════════════════════════
function LAB.Summary()
    local out={}
    out[#out+1]="=== CheatMenu 实验室 · 扫描摘要 ==="
    out[#out+1]="时间: "..os.date("%Y-%m-%d %H:%M:%S")
    local c=LAB.Caps()
    ...
    if #LAB.Log>0 then
        out[#out+1]=("调用记录: %d 条(最近一条: %s)"):format(#LAB.Log,LAB.Log[#LAB.Log])
    end
    ...
    if ok then SYS.Notify("摘要已复制到剪贴板",SYS.CY.green)
    else print(txt) SYS.Notify("这台执行器没有剪贴板, 已打到控制台") end
    return txt
end
...
UI.Pages["实验室"]=function(p)
    UI.Label(p,"🔬 实验室 · 游戏内部探查",CY.purple)
    ...
    UI.Section(p,"一键(推荐)",CY.green)
    UI.Btn(p,"🔍 综合扫描: 通信层 + 代码层 + 脚本层",CY.green,function() P(function() SYS.Lab.FullScan() end) end)
    UI.Btn(p,"📋 复制本次扫描摘要到剪贴板",CY.cyan,function() P(function() SYS.Lab.Summary() end) end)
    UI.Tip(p,"一次点完, 把三件事全扫出来打到控制台(F9):\n  A 通信层 = 游戏有哪些 Remote(能触发什么) —— 和设置页那个扫描是同一个\n  B 代码层 = 游戏有哪些函数(怎么实现的) + 名字可疑的(damage/fire/aim…)\n  C 脚本层 = 游戏跑了哪些脚本/模块",CY.sub)
    UI.Div(p)

    UI.Section(p,"单点(想单独看某一层时)",CY.cyan)
    UI.Btn(p,"① GC 扫描(函数/表 + 按归属脚本分组)",CY.cyan,function() P(function() SYS.Lab.ScanGC() end) end)
    UI.Btn(p,"② 可疑函数(名字含 damage/fire/aim/kill…)",CY.cyan,function() P(function() SYS.Lab.ListHookable() end) end)
    UI.Btn(p,"③ 已加载脚本 / 模块清单",CY.cyan,function() P(function() SYS.Lab.ListScripts() end) end)
    UI.Tip(p,"结果都在【控制台(F9)】, 方便你复制发我。",CY.sub)
    UI.Div(p)
...
UI.Pages["设置"]=function(p)
    ...
    UI.Tip(p,"只扫 Remote 这一层。想一次把【Remote + 游戏函数 + 脚本】都扫出来, 去「🔬 实验室」点【🔍 综合扫描】。",CY.sub)
    UI.Div(p)
