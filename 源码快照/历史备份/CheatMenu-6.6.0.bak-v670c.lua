...
local SYS={
    ...
    T_={
        ...
        -- 瞄准补强
        CB_Sticky=false,CB_MissMode=false,
        -- 默认关闭的三项(用户点名: 加入但默认关)
    ...
    C_={
        ...
        CB_HitRate=100,CB_StickyHold=1.5,CB_MissRate=0,
...
local function aimTick()
    ...
    -- ★ v6.7.0 瞄准补强(融合自 ChronixHub 瞄准设置): 粘性保持 + 命中率闸门
    if SYS.AimEx then
        if SYS.AimEx.StickyOK() and not CB.TargetPart then
            local sp=SYS.AimEx.StickyPart
            if sp and sp.Parent then CB.TargetPart=sp end
        end
        if not SYS.AimEx.HitGate() then return end
    end
...
--   玩家控制(动手类) / 整蛊工具 / 工具增删
--
--   ⚠ 本块内【不得引用 UI.*】—— UI 库定义在后面, 这里引用会拿到 nil。
--     需要界面的一律在页面函数里做(见 [14] UI 页面)。
--
...
do
    ...
    Aim.StickyPart=nil Aim.StickyAt=0

    function Aim.StickyOK()
        if SYS.T_.CB_Sticky~=true then return false end
        local p=Aim.StickyPart
        if not p or not p.Parent then return false end
        local hold=SYS.C_.CB_StickyHold or 1.5
        if (os.clock()-Aim.StickyAt)>hold then return false end
        return true
    end

    function Aim.NoteTarget(p)
        if p then Aim.StickyPart=p Aim.StickyAt=os.clock() end
    end
...
UI.Defs={
    ...
    {name="玩家",icon="👤"},{name="整蛊",icon="😈"},{name="传送",icon="➲"},
    {name="挂机",icon="★"},{name="MachineParty",icon="🎮"},{name="翻译",icon="🌐"},{name="设置",icon="⚙"},
}
...
UI.Pages["战斗"]=function(p)
    ...
    UI.Switch(p,"🩹 粘性瞄准 (目标出圈也先粘住不换人)","CB_Sticky")
    UI.Slider(p,"粘性保持时间 (秒)",0.3,5,0.1,
        function() return SYS.C_.CB_StickyHold end,
        function(v) SYS.C_.CB_StickyHold=v end,"%.1f")
    ...
    UI.Tip(p,"「粘性瞄准」= 目标短暂离开索敌圈时先不换人, 减少左右横跳。\n「命中率」= 只有 n% 的帧去转相机; 100 最准, 调低后更接近人手。\n「漏打模式」= 开火前按概率跳过一枪, 避免每枪都爆头的观感。\n★ 平滑度/预判量/索敌半径已经在上面 —— 对应「跟随速度 / 预测提前量 / 索敌范围」, 不再重复给控件。",CY.sub)
