...
local SYS={
    ...
    T_={
        ...
        -- ★★ v9.10.1 通用按键自动化(小游戏里的 WASD / 方向键自动按)。默认关。
        --   实现见 SYS.Keys; 参数 Key_Which / Key_Mode / Key_Gap 声明在 C_ 表里。
        Key_Auto=false,
    ...
    C_={
        ...
        -- ★★ v9.10.1 按键自动化参数(界面在 MachineParty 页「⌨ 自动按键」区)。
        --   必须在 SYS.C_ 里声明 —— SaveConfig 只遍历【声明表】的键,
        --   不声明 = 界面调了写不出去、重载回默认(这个坑 CB_ScanMs 踩过, 见它的注释)。
        Key_Which="W",          -- 要按的键(Enum.KeyCode 的名字; 可用 + 连接多个, 如 "W+D")
        Key_Mode=1,             -- 1=按住  2=连按(点一下)
        Key_Gap=0.05,           -- 连按模式的间隔(秒)
...
do
    ...
    --================================================================
    -- ★★ v9.10.1 新增: 通用按键自动化 (SYS.Keys)
    --   为什么加: 用户问「机械那个游戏, 需要 WASD 的电梯 / 看闪电上下左右的, 能全自动吗」
    --     —— 全菜单唯一会"按键"的地方是近战补刀按 F(L8681), 没有通用按键自动化。
    --   能力: 按住 / 连按(点一下)。底层就是补刀用的同一套 API:
    --     SYS.VIM:SendKeyEvent(down, Enum.KeyCode.X, false, game)
    --   ★ 明确不做的: 不去猜"该按什么键"。按哪个键由用户在界面选 ——
    --     要"看提示自动按"(闪电那类)必须先扫出那 74 个 InputBinding 的路径,
    --     没样本就猜着按 = 违本项目铁律(先拿真实样本, 再接判据)。
    --   ★ 安全: 关闭/卸载一律 ReleaseAll() —— 绝不把某个键留在"按住"状态,
    --     否则角色会一直朝那个方向走, 玩家会以为"脚本把键盘锁死了"。
    --================================================================
    local K={} SYS.Keys=K
    K.Held={}                 -- [键名]=true(正在按住), 用于无遗漏地松开
    K.LastTap=0
    K.Stat={press=0}
    ...
    function K.Tick()
        ...
        -- ★★ v9.10.1 支持【一次按多个键】: 键名可以用 + 连接, 例如 "W+D" / "W+A"。
        --   为什么要: 斜向移动、电梯+方向键同时、以及有些玩法要"前进+抓"这种组合,
        --   一次只能按一个键就做不了。解析失败(键名写错)时什么都不按 —— 不会乱按。
        local want={}
...
UI.Pages["MachineParty"]=function(p)
    ...
    --================ ★★ v9.10.1 新增: 通用按键自动化 ================
    --   这是本页原来【唯一缺】的那一类: 上面两个是"触发型/回避型",
    --   而"需要按住 WASD 的电梯""看提示按上下左右"属于【输入型】——
    --   全菜单原来只有近战补刀按 F 会发键盘输入, 没有通用的按键自动化。
    --   底层就是同一套 API: SYS.VIM:SendKeyEvent(down, KeyCode, false, game)。
