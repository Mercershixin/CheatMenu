...
--#   ★ 完整规范 = 仓库根目录 AGENTS.md（唯一权威；本块是它的精简版）
--#   仓库里那份 CheatMenu.lua 是【发行产物】(构建时去注释+压缩), 手改请改本地 CheatMenu-<版本>.lua
--#   📌 当前源码版本 = 12.2.0
--#      ★ 必须与 build/VERSION、以及本文件名 三者一致(check.py 有锁在盯这一行, 改号时一起改)
--#
--#  1) 只做用户明确要求的那一件事。不擅自新增功能、不改用户没让动的地方。
--#  2) ⛔ 不擅自跑测试(luau-compile / verify_* / check*.py 都算)。用户说"跑一下/验证/测试/发版"才跑。
--#     ★ 发版本身跑 push_now.py 属交付步骤, 不算额外验证循环。
--#     ⛔ 整脚本仿真 / mock 环境 / 循环检测 一律禁跑、禁修、禁扩展。
--#  3) ⛔ 接口红线: 单点 hookfunction 可以; hookmetamethod(game,"__namecall"/"__index")
--#     做关键词搜索【绝对不行】—— 那是所有实例方法调用的总入口, 卡顿掉帧的病根。
--#     不主动改元表、不加 setreadonly。
--#  4) UI 标"不推荐/有风险"是给用户看的提示, 不是让你改行为。
--#  5) ⛔ 已删功能不要"顺手加回来"(清单见 AGENTS.md §3)。
--#
--#  6) ⛔⛔ 「移动」与「过检测」必须是两个模块, 别合:
--#     · 移动(飞行/加速) = 本地动多快, 单位【格/秒】(C_.FlyAbs / C_.SpeedAbs, 0 = 用倍率)
--#     · 防回退(过检测) = 上报给服务端多快 (C_.RevertCap; T_.AntiRevert)
--#     两者天生拉扯: 上报上限低于实际速度 -> 服务端把你拽回, 看着像"定在原地"。
--#     ⛔ 任何"速度上限/阈值"都不能写死常量。
--#  7) ⛔ Roblox 里所有 RemoteEvent 的 FireServer 是【同一个 C 函数】——
--#     "逐 remote 各 hook 一次"做不到; 只能 hook 共享那一个, 调用时按 self.Name 判断。
--#  8) ⛔ 任何 hookfunction 的模块必须同时挂进 UnloadAll(或 FuseClean)。
--#  9) ⛔ 界面容器固定挂 PlayerGui(gethui/CoreGui 会让菜单偏 58px)。
--# 10) ⛔ UI.Slider 的 get() 返回值必须落在 [min,max] 内(否则会被静默夹到边界)。
--# 11) ⛔ 中文字符串里绝不能用 ASCII 双引号(会截断字符串) —— 一律用 「」。
--#
--#  ┌────────────────────────────────────────────────────────────────────┐
--#  │ ★★ V3 内核（12.0.0 起）—— 新增/改动的都在这一段, 其余功能体一字未动 │
--#  └────────────────────────────────────────────────────────────────────┘
--#  ① SYS.K.Sched  统一调度器: **一个信号只开一条驱动连接**。
--#     旧实现: 每个 SetLoop 各连一条 Heartbeat, 且「传进去的是新闭包」时
--#     会先断线再重连(拨一次开关抖一下)。现在按「名字」复用任务, 只换函数体。
--#  ② SYS.K.Guard  错误显形: 任何任务/回调第一次出错就打到控制台 + 进错误清单。
--#     ⛔ 这是治那类"功能静默无效"的病根 —— 以前 P(...) 吞了错, 看着像没生效。
--#  ③ SYS.K.Hook   hook 归属 + **后进先出**还原。卸载顺序不再是逻辑(旧实现靠人记)。
--#  ④ SYS.K.Bus    连接归属登记: 谁挂的一目了然, 能整组摘掉, 卸载不漏。
--#  ⑤ SYS.K.Net    事件图谱 + 下行监听。★ 关键: 远程**还没下发**时先挂起,
--#     下发后自动重绑 —— 这是以前"监听事件缺失"的主因(旧 OnRemote 拿不到就直接 return)。
--#  ⑥ SYS.K.Watch  以前缺的监听面: 玩家/角色/死亡 · Attribute · 值对象 · 标签 · 实体出现。
--#  ⑦ SYS.K.Idle   空闲静默: 挂起自建循环 + 摘掉监听面, 压小动态足迹(不是隐身, 只是少留痕迹)。
--#  ⑧ 【一个扫描功能】= SYS.Scanners + SYS.RegisterScanner + SYS.ScanAll,
--#     UI 上只有「🔍 综合扫描」一个按钮。12.1.0 起把「🛡 能力绕过判定」(飞行加速/回血/锁血/
--#     上帝/高亮/透视/传送 逐条判定)·「🧩 事件扫描」(V3 图谱+监听面+运行态)·
--#     「🧪 自检合集」·「🕵 外部手法对照」全部注册进同一个扫描 —— 别再新开扫描按钮。
--#  改 V3 之前先看仓库 AGENTS.md §5; 走 SYS.K 的接口, 别再往 SYS 上挂新全局。
--#  ⚠ 内核只留**真被调用**的接口: 12.1.2 删掉了 4 个零引用 API
--#     (K.LogCats / BUS.Prop / BUS.Attr / HK.RestoreOwner)。要加新接口请同时接上调用点,
--#     否则 _audit_features2.py 的 E 段会把它算成死代码。
--#
--#  🎨 配色: 高亮一律"边框高亮"(填充 0.88 / 隔墙 0.93, 只留描边); 普通物件+普通门=亮青(0,200,255);
--#          危险(陷阱/伤害机关/切割/假门)=红(255,30,30)+☠; 人物=队友绿/敌人红/幽灵紫; 怪物NPC=橙。
--#          「掉落物/可交互/门·陷阱·假门」已合并成「🔍 物件透视」一个开关, 别再拆开。
--#  🚀 发版: 改源码 -> dist/repo/CHANGELOG.md 顶部写条目 -> push_now.py --fast [--patch]
--#          -> local_sync.py -> push_docs.py -> 核对 version.txt == CHANGELOG 顶部 == 远端实际文件。
--#  🗂 定位: 用符号名 grep(不要抄行号, 每次改源码都会漂移); 分层见 AGENTS.md §2。
--############################################################
--
-- ┌──────────────────────────────────────────────────────────────────────────┐
-- │ 模块索引（V3 · 只列层与关键符号, 不写行号 —— 行号每改一次就漂)            │
-- └──────────────────────────────────────────────────────────────────────────┘
--  [0] 内核       SYS.P SYS.T SYS.TT SYS.Notify SYS.Orig          ← 旧内核(保留)
--                 SYS.K.{Log,Guard,Sched,Bus,Hook}                ← ★ V3 新内核
--  [1] 配置       SYS.T_（开关）· SYS.C_（数值）· Save/LoadConfig · REMOVED_FEATURES
--  [2] 通信       SYS.REvent REventU RFunction Fire FindEvent RemoteAlias RemoteRisk
--                 SYS.K.Net.{Atlas,Sig/Attach,Bind,Resolve,Rebind,Spy,Categories} ← ★
--  [3] 检测面     SYS.ACBlock AntiRevert Prot AC(D1~D14) · SYS.K.Idle ← ★
--  [4] 移动       SYS._Move FlyTick SpeedTick CleanFly CleanSpeed SetNoclip TPTo
--  [5] 视觉       SYS.NewVis espFill Gun RayHook HitMark* SetFreeCam HUD_Info
--  [6] 战斗       SYS.Combat(CB) FP DeadOnTime
--  [7] 玩法       SYS.DRHp DRCombat Trans AutoDodge … · SYS.K.Watch ← ★
--  [8] 界面       UI.Pages UI.Switch UI.Slider UI.Cycle UI.Btn UI.Tip EnsurePage
--  [9] 编排       SYS.CreateMenu UnloadAll BootUpdateCheck CheckUpdate · SYS.V3Boot/V3Teardown


-- ================================================================================
-- [1] 配置（T_ / C_ 默认值 + 存档读写；本段为自动判定的近似标签）
-- ================================================================================
--############################################################
--# ⛔ 改这个文件之前【必读】—— 任何 AI / 智能体, 不管哪个账号
--#   ★ 完整规范 = 仓库根目录 AGENTS.md（唯一权威；本块是它的精简版）
...
local SYS={
    ...
    C_={
        ...
        -- ★ 2026-09-25 门禁(_audit_c_)逮到: 下面这几个数值一直【被读/被写, 却没在 C_ 表里声明】——
        --   LoadConfig 的还原分支带 `SYS.C_[k]~=nil` 守卫 ⇒ 没声明的键【存了也读不回来】,
        --   用户改过的值重载后必丢。补进 C_ 表 = 数值类配置的「四点」第 1 点。
        --   ⚠ LightMode 故意声明成空串 "": MigrateLightMode 的迁移条件正是 `==nil or ==""`,
        --     写成 "关闭" 会让【老存档的光照设置迁移失效】(AGENTS 专门警告过这个坑)。
        --   ⚠ GameNameCache 是懒填缓存; 声明成 "" 后, 判空条件已同步改成 `==nil or ==""`。
        AtlasCap=60000,
        CamZoom=20,
        -- ★ 12.1.6 删: 这里原有 `HitDist=30` —— 声明后【全文件从未被读过】的死键
        --   (12.1.4 删「命中标记整簇」时漏网的一枚配置项)。
        LightMode="",
        WL_Sel="",
        GameNameCache="",
...
-- ================================================================================
-- [0] 内核 V3 —— 统一调度 / 事件总线 / 错误显形 / hook 归属 / 事件图谱
--   ★ 与旧内核并存: SYS.P / SYS.T / SYS.SetLoop / SYS.SpawnLoop / SYS.SafeHook
--     全部**保留原名**(调用点一行不用改), 内部改走 V3。
-- ================================================================================
do
    local K={ Ver="V3" }
    SYS.K=K
    ...
    function SC.Del(name)
        -- ★ B2 修: 旧实现当场 table.remove(SC.order) —— 而它经常是在 Pump 遍历里被调的
        --   (某个任务自己关自己) ⇒ 索引前移, 当帧会【静默跳过后面的一个任务】。
        --   改成"只打死亡标记", 真正的压缩放到下一帧 Pump 开头(那时没在遍历)。
        local rec=SC.jobs[name]
    ...
    function BUS.Raw(conn,tag,owner)
        -- ★ B1 补: 给"自己手工 Connect 出来的连接"也上户口 —— 否则 BUS.Off 摘不掉它
    ...
    function NET.Track()
        ...
        for i=1,#rt do
            BUS.Sig(rt[i],"DescendantAdded",function(d)
                ...
                -- ★ B4 修: 三个"监听面"以前只在开开关那一刻扫一遍 —— 之后新出现的实例永远漏掉。
                --   这里补上"新来的也接上", 才配得上「实体出现 · 不用轮询」这句承诺。
                local Wc=K.Watch
                if Wc then
    ...
    -- ★ 关掉监听面时必须把「已登记」一起清掉, 否则再打开会因为"已登记"而不再重绑
    --   —— 这正是本项目反复踩的那类"静默无效"。前缀只清实例级键, 不动 players/attrs 这类总开关。
    ...
    function W.Hum(p,ch)
        ...
        if not h then
            -- ★ 以前缺的一环: Character 先到、Humanoid 后到(很常见) ⇒ 这里补上「等 Humanoid」
    ...
    function SYS.V3Boot()
        ...
        -- ★ 12.1.2 修(门禁 _audit_features.py 逮到): PerfProfile 只有 UI 的 onChange,
        --   没有"启动时补一次" —— 存档里是 true 的用户, 重载后界面显示"开"、实际没开
        --   (= 本项目反复踩的「开了没反应」)。按"四点"那条规矩在这里显式补挂。
...
Prot_SpeedCap=true }
    ...
    function SYS.SetLoop(k,on,sig,fn)
        -- ★ V3: 走统一调度器。旧实现每次传新闭包都会「断线 -> 重连」(拨一下开关抖一下),
        --        且一个任务一条 Heartbeat 连接(几十条)。现在按名字复用, 一个信号一条连接。
        if on then
            if not sig then return end
            -- ★ B3 修: 空闲静默开着时调度器是挂起的 => 新加的任务永远不会跑(=开了没反应)。
            --   开功能就自动退出静默, 并把开关状态同步回 UI(否则界面显示"开"而其实是关的)。
    ...
    function SYS.SpawnLoop(fn)
        -- ★ V3: 出错不再只 warn 一句 —— 走错误显形(控制台首报 + 进错误清单)。
...
do
    ...
    function SYS.OnRemote(n,cb)
        -- ★ V3: 旧实现在 remote 还没下发时直接 return(监听**静默丢失**) ——
        --        现在先挂起, 远程一下发就自动绑上(这是「监听事件缺失」的主因)。
...
do
    -- ================== 移动核心（飞行 / 加速 共用）· 重写 ==================
    -- 三条原则:
    --   ① 只管「让角色本地动起来」; 速度一律【格/秒】绝对值(老的倍率仍兼容: 绝对值为 0 时才用倍率)。
    --   ② 三种驱动各自独立、互不污染 —— 不再有 SYS._FlyDegraded 那种全局粘性开关。
    --   ③ 关掉 / 换角色 -> 挂上去的实例全部销毁, 一个不留。
    --   ⚠ 「防回退(过检测)」是【另一个模块】(见 SYS.AntiRevert):
    --      它管"上报给服务端的位置看起来合不合理", 不管本地动多快。两者故意分开。
    ...
    -- ★ 修(门禁逮到): 这 4 个名字以前是【裸全局】——
    --   ① 漏进 getgenv() 变成可检测指纹; ② 同一个执行器里重载脚本会粘上一次的值。
    --   check.py 里本来就有一条锁要求 `local NoclipAtt,NoclipVel=false,false`, 说明是改动中丢的声明。
        ...
        noclipThread=task.spawn(function()
            while SYS.T_.Noclip and not SYS.Unloaded do
                ...
                if c then
                    ...
                    -- ★ 修(真 bug): 穿墙会在角色 root 上自建 Attachment + LinearVelocity(MaxForce=math.huge)。
                    --   飞行开着时, SYS._Move 的驱动也在同一个 root 上 —— 两个 LinearVelocity 互相抢,
                    --   表现为速度抖 / 干脆推不动。飞行期间把穿墙自己那份收掉。
                    --   (AGENTS §2 硬规则④: [4] 层的驱动实例只允许 SYS._Move 创建)
                    if SYS.T_.Fly then
                        if NoclipVel then NoclipVel:Destroy() NoclipVel=nil end
                        if NoclipAtt then NoclipAtt:Destroy() NoclipAtt=nil end
...
do
    ...
    function SYS.UnhookAllSafe()
        ...
        -- ★ V3: 兜底按后进先出再还原一遍(旧实现是 pairs 顺序 = 卸载顺序靠运气)
    ...
    SYS.RegisterScanner("🩺 一致性 / 只读体检 (时间函数 / game 元表 / 网络所有权 / FPS·GC / 反作弊框架 / 反挂机)", function()
        -- ★ 12.1.4: 这几项原先只由「反指纹自检」调用; 那个自检的 UI 在 11.8.6 按用户要求删了,
        --   它们就成了零引用孤儿。**没有删** —— 它们是真有用的只读体检, 这里接进统一扫描
        --   (扫描是控制台输出, 不占 UI 任何位置)。
        local o={}
    ...
    SYS.RegisterScanner("游戏信息 / 执行器能力 (实测 · 决定本机能开什么)", function()
        -- ★ 12.2.0: 这个扫描器原来是个空壳(只返回一行「见头部」)。填成真探测。
        --   为什么值得实测而不是"有就用": 同一份脚本在不同执行器上能力差很多, 而
        --   能力缺失的表现都是【静默退化】(没有 mousemoverel 就退回相机路径, 没报错)。
        local o={}
        local UA=SYS.UA
        ...
        o[#o+1]=""
        o[#o+1]="── 队伍字段 ──"
        -- ★ 这里【不能直接写 `CB`】: 本扫描器在 `local CB=SYS.Combat`（后面才声明）之前,
        --   直接引用它会变成"在 local 声明之前引用"= 未知全局, 正好被 check_globals 逮住。
        --   走 `SYS.Combat` —— SYS 是早声明的作用域根, 且扫描器是运行期才执行。
        do
...
function CB.PickVisiblePart(pl,mode)
    local ch=pl and pl.Character
    if not ch then return nil end
    -- ★ 12.2.0 通用化: 先找【游戏自带的瞄准命中盒】—— 它才是游戏自己打的目标
    --   (DoW 的 AutoAimAreaHead/Body · Counter Blox 的 HeadHB), 比可见头更准。
    --   没有就原样走下面的写死名单, 行为不变。
...
local function pickTarget()
    ...
    -- ============================================================================
    -- ★ 12.2.0 通用投递层 SYS.UA —— 只学【手法】, 不抄代码
    --   来源(公开仓库, 2026-09-25 调研): seltonmt012/sel01-rbx(universal.lua / phantomforces.lua)
    --   · ttwizz/Open-Aimbot · Exunys/AirHub-V2。逐条裁决见
    --   事件库/2026-09-25_通用化与公开源码技法落地_12.2.0.md。
    --   --------------------------------------------------------------------
    --   为什么要有它: 原来 aimTick 只【写 cam.CFrame】。很多游戏每帧从自己内部的角度重建相机
    --   (实测 Phantom Forces / BloxStrike), 写入下一帧就被丢 ⇒ 自瞄「看着在动、其实没生效」,
    --   而面板还诚实地说「相机」。通用的活路是【走真实鼠标】: 让游戏自己的相机控制器执笔,
    --   两种游戏都吃, 且运动形态天然是人的。
    -- ⛔ 不抄: 全局 __index/__namecall 劫持 · Drawing 当默认 · 任何 webhook 上报。
    -- ============================================================================
    do
        local UA={}
        SYS.UA=UA

        -- ① 执行器能力: ★ 绝不用 rawget(getfenv(),「mousemoverel」)
        --    Potassium 类执行器把全局通过 env 的 __index 交出来, rawget 拿到 nil,
        --    于是静默退化成相机路径 —— 原版 universal.lua 就栽在这里, 白丢整个自瞄。
        ...
        -- ② 游戏自带瞄准命中盒识别
        --    判据: BasePart 且名字含 AutoAimArea / Hitbox / HB_ 之类; 名字里带 head 的算头盒。
        --    ⚠ 不用 GetBoundingBox 做兜底: 实测 Counter Blox 会返回比真实大 10 倍的盒子。
        ...
        -- ③ 相机写入存活率探测 + 鼠标灵敏度自学习 + 投递
        --    存活判据: 我们上一帧写的朝向, 本帧还差 <0.12 度 ⇒ 活; 否则被游戏重建掉了。
        --    ⚠ 只统计「本帧我们自己没动鼠标」的样本 —— 否则量到的是我们自己。
        ...
        -- 人类化: 反应延迟 / 每秒角度上限 / 死区 / 【平滑随机游走】噪声
        --   ★ 游走不是每帧白噪声 —— 白噪声在相机上读起来是「抖」, 游走读起来是「手」。
        ...
        function UA.Deliver(cam,want,cf,spd)
            ...
            -- 选投递方式: 强制鼠标 / 强制相机 / 自动(相机写入存活率 <50% 且鼠标可用就切)
            ...
            -- 鼠标路径: 把「想要的朝向」换算成鼠标请求量
            local wp,wy=want:ToOrientation()
            local kY=(sensY>0) and sensY or 0.0016   -- 还没学到时用保守初值, 1~2 帧就收敛
            ...
            -- ★ 亚像素请求会被系统抹掉 ⇒ 跳过, 免得从「没发生的移动」里学习灵敏度
...
local function aimTick()
    ...
    -- ★ 12.2.0 通用投递层: 能走鼠标就走鼠标(让游戏自己的相机控制器执笔),
    --   否则写相机。原实现只写相机 —— 在「每帧重建相机」的服上写入会被丢,
    --   表现为自瞄看着在动其实没生效。
...
function CB.MigrateV72()
    ...
    -- ★ 12.1.5 修(无效补丁): 原来写的是 `or 0.08)==0.06` —— 兜底值与比较值不一致,
    --   老存档没设过这条时兜底成 0.08 ⇒ 永远等不到 0.06 ⇒ 这一行【从未生效过】。
    --   本意 = 只把「等于旧默认 0.06」的升到 0.08, 用户手调过的一律不动
    --   （旁证: L14079 读这条时的兜底正是 0.06）。
...
do
    ...
    -- ★ 12.1.6 删(用户拍板): 这里原来是 `SYS.SetAntiFling`(防甩飞)。AGENTS §3 曾把它列入
    --   「用户明确说还需要, 别再删」；但它【既没有界面入口、`AntiFling` 也没在 T_ 表里声明】,
    --   启动恢复(遍历 `SYS.SwitchOnChange`)也带不动它 ⇒ **全项目零引用, 从来用不到**。
    --   用户 2026-09-25 明确答复「直接删掉」⇒ 连同 `FLING_MAX` / `SYS._flingN` 一起移除
    --   （后两者经 grep 确认只在这一个函数体内出现）。
    local WL={} SYS.WL=WL
    WL.White={} WL.Black={}
...
do
    ...
    -- ★ 2026-09-24: 把「移动环境」并进【总扫描】（SYS.Scanners 那套）——
    --   与移动页的「🧪 移动自检」共用同一个数据源 SYS.MoveDiag(), 不重复实现。
    -- ★★ 2026-09-24: 「看控制台」入口全部并进【总扫描】
    --   做法: SYS.CaptureLines(fn) 临时接管 print 收集输出 —— 这样【不用重构】那些
    --   只管 print 的旧 dump 函数, 就能让它们出现在总扫描里。
    ...
    -- ★★ 2026-09-24: 【常驻控制台镜像】—— 一劳永逸解决"一大堆功能各自打到控制台"。
    --   做法: 给全局 print 挂一层"顺带记录"的壳(环形缓冲 300 行), 再注册成一个扫描层。
    --   ⇒ 任何旧功能(不管是谁写的、改没改过)只要 print, 都会自动出现在【总扫描】里,
    --     不需要逐个改它们, 也不需要在 UI 里保留一堆「看控制台」按钮。
    --   开销: 每条 print 多一次 Lua 调用(print 本身是低频操作) —— 可忽略。
    do
...
-- ★ 12.1.5 正名: 原叫 CreateMenuLegacy —— 但全项目**只有这一个实现**（`CreateMenu()` 只是
--   把它 return 出去），"Legacy" 是当年"做过新版又回退"留下的历史名，容易让人以为还有别实现。
