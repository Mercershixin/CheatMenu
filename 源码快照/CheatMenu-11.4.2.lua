...
do
    ...
    local function buildNames()
        ...
        for i=1,#list do
            ...
            -- B.1 做什么: 建"移动通道"名字表。为什么: 后面只对这些通道的【数值】参数做夹值。
            --   只登记【上行】—— ServerReplicateCFrame 是服务端下行、不该管; 名字按"末段"和"全串"两种都放,
            --   因为 remote 实例的 .Name 通常只有末段(如 WalkSpeed 而不是 EntityService.WalkSpeed)。
            ...
            AR.orig=hookfunction(ev.FireServer,newcclosure(function(self,...)
                ...
                for i=1,n do
                    ...
                    elseif tp=="Vector3" then
                        ...
                        -- B.1 做什么: 模长≈1 的 Vector3 判为"朝向/单位向量"(如 EntityService.PitchYaw) -> 直接放行,
                        --   并且【不更新 lastPos】。为什么: 老代码对任何 Vector3 都写 lastPos, 一个朝向向量会把基准
                        --   污染成单位长度, 之后所有位置限速都算错 —— 这是顺带修掉的真 bug。
                    ...
                    elseif tp=="number" then
                        -- B.1 做什么: 只把【8 条移动通道里、名字像速度/跳跃的】数值参数夹到 cap 以内。
                        --   为什么加两道条件: 不加 isMove/speedLike 会把 SetState 这类枚举值一起夹了;
                        --   只夹不抬(v>cap 才动), 所以正常值一定原样通过。关掉 AntiRevertExtra 即完全恢复原行为。
