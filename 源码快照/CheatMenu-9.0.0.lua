...
do
    ...
    --   与 SaveRemotes 同一套命名规则(标签_游戏名_PlaceId_时间戳), 文件名安全化复用上面的 safeFileName。
    ...
    function SYS.ResolveScanDir()
        ...
        -- ★★ v8.11.0: 文件夹名【优先用真名】(满足"文件夹是服务器的名字"),
        --   只有当"真名路径"写不进去时才退回 ASCII 版本。
        --   依据: 用户桌面实测里 `Remotes_[火爆全球的囧次元]_...` 显示是【正常】的,
        --   说明执行器写中文名【并非必然乱码】(乱的是另一批) —— 所以不该一上来就放弃中文。
        --   真名路径万一乱码/写失败, 还有同级的 `服务器名.txt` 兜底, 不会认不出来。
        ...
        local cands={
            "C:\\Users\\Administrator\\Desktop\\"..pkgCN,    -- ① 用户点名的「包」+【真名文件夹】(第一优先)
            "C:\\Users\\Administrator\\Desktop\\"..pkg,      -- ② 「包」+ ASCII 文件夹
            "C:\\Users\\"..who.."\\Desktop\\"..pkgCN,         -- ③ 用户名 != Administrator 时
        ...
        for _,dir in ipairs(cands) do
            ...
            if ok and isfile(dir.."\\_probe.txt") then
                ...
                -- 文件夹里留一份【真名】(内容用 UTF-8 写, 不受文件名编码问题影响)
                pcall(function()
    ...
    function SYS.DumpScanAll(buf)
        ...
        local info,nm,pid=SYS.GameInfoLine()
        local stamp=os.date("%Y%m%d_%H%M%S")
        -- ★★ v8.11.0 文件名【纯 ASCII】(治"中文文件名乱码"): scan_<PlaceId>_<时间戳>.txt
        --   真正的中文服务器名: ① 文件头这几行里有(内容编码正常) ② 文件夹里的「服务器名.txt」
