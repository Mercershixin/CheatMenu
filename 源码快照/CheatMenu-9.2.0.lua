...
local SYS={
    ...
    T_={
        ...
        -- ★★ v9.2.0 新增: 服务端播报监听(倍率/Boss/糖果/天气/消息) —— 纯监听, 默认开
        SrvWatch=true,
        ...
        --   360 无死角 / 无延迟模式 / 真·静默(不转相机) / 翻译中英对照 / 自动找低延迟服务器
        CB_360=false,CB_SilentNoTurn=false,
        NoAggro=false,      -- ★ v6.9.24 无仇恨模式(隐身+免伤+自动躲 一键开)
        TransBilingual=false,
        TransDyn=true,
        TransSili=false,     -- ★ v120.1 翻译后端: false=本地 llama.cpp / true=硅基流动云端 API
        AutoLowPing=false,
...
-- ★ v6.9.0 通用化: 走 SYS.FindEvent（别名 + 关键词模糊扫描），不再硬编码路径。
--   可选 pl: 传了就当作目标参数带上（很多游戏的 Heal/Revive 接受目标），
--   先试带参、再试无参 —— 服务端对不认识的参数一般直接忽略，不会出事。
-- ★ v6.9.0 增强: 多候选轮询。
--   同一个功能在游戏里往往有好几条通道(Heal / RequestHeal / EntityService.Heal / Regen…),
--   其中有些是**免费**的。以前只试"第一个命中的"就收工; 现在把候选挨个试一遍, 提高命中免费通道的概率。
--   ⚠️ 能力边界: 若某条通道**服务端要求消耗**(药水/道具), 客户端是绕不过的 —— 服务端在扣。
--      能做的只是"多找几条路, 尽量走免费那条"。
local function _fireEvent(kind, pl)
    local tried = {}
    local cands = {}
    -- ① 先把别名表里**所有**候选都排上（FindEvent 只返回第一个命中的）
...
-- 一键 0 元扫货: 列出游戏里【价格 = 0】的可购买商品(Gamepass / DevProduct)并尝试购买。
-- ⚠️ 只碰标价 0 的 —— 付费的一个都不会动。
function SYS.FreeSweep()
    local n0, bought = 0, 0
    local mps = game:GetService("MarketplaceService")
    -- ① Gamepass: 从游戏的 Universe 里拿不到完整列表, 这里只对"地图上已有的购买入口"生效
    -- ② 更实际的做法: 直接调 buy 类 remote 并传 0 价商品 id —— 但 id 得知道, 所以先扫描远端的商品提示牌
    local r = SYS.FindEvent("buy")
    if not r then
        SYS.Notify("⛔ 本游戏没找到 buy 类 remote", SYS.CY.yellow)
        return 0
    end
    -- 扫描 Workspace 里带 ProductId / GamepassId 的互动物件(很多游戏把商店做成物理件)
    local ids = {}
    pcall(function()
...
do
    ...
    -- ★ v6.11 注: 关键词越多, 挂上的 OnClientEvent 越多(HUD 播报有 8 秒去重, 不会刷屏)。
    --   若某游戏 remote 特别多, 可用下面的 SetEventWatch 通知里的"挂上 N 条信号"看数量。
    ...
    -- 文件名安全化: 只替换 Windows 非法字符, 中文/字母/数字都保留
    ...
    function SYS.ResolveScanDir()
        ...
        local cands={
            "CheatMenu\\"..folder,   -- ① workspace\CheatMenu\<PlaceId>_<ASCII名字>  (推荐, 已实测可写)
            folder,                  -- ② workspace\<PlaceId>_<ASCII名字>
            ".",                     -- ③ workspace 根
        }
        ...
        for _,dir in ipairs(cands) do
            ...
            if ok and isfile(dir.."\\_probe.txt") then
                ...
                -- 文件夹里留一份【真名】
                -- ★★ v9.2.0 修: 原来叫「服务器名.txt」—— **实测被执行器写花了**
                --   (`服务器名.txt` 实际落到磁盘 = `鏈嶅姟鍣ㄥ悕.txt`)。文件名一律 ASCII。
                pcall(function()
    ...
    --#   用法: SYS.Hud("文本", 秒数)；秒数留空=4 秒；传 0=常驻。
    --#   用途: 更新提示 / 事件预告 / 关键状态播报。
    --##########################################################################
    ...
    -- C4 · 背包读取：把能拿到的背包/物品信息打到控制台 + 文件
    function SYS.DumpInventory()
        local out = {}
        local rel = RStorage:FindFirstChild("Remote")
        -- ① 尝试用 RemoteFunction 直接问服务端要背包
        local got = false
    ...
    -- C3 · 服务器列表：读 ServerList / 服务端下发的数据
    ...
    -- C3 · 快速换服：把最后读到的服务器列表里第一条可用 ID 拿去 Join
    ...
    --#      客户端做不到 —— 客户端能做的拦截见 Prot_AntiAC / TrapImmune(本地免伤)。
    --##########################################################################
    ...
    --##########################################################################
    --# ★★ v6.9.0 道具 / 装备（别名全部取自事件库）
    --#
    --#   ⛔ 先把「免道具」说清楚 —— 别再当 bug 查:
    --#      使用道具 / 装备 / 合成 **都是服务端权威**。客户端发 TryUse(id) 之后,
    --#      服务端查你的背包: 有没有这个道具、够不够次数、冷却过没过, 不够直接拒绝。
    --#      **客户端没有合法途径"免掉"这个消耗** —— 那要改服务端(做不到), 或游戏自身有漏洞。
    --#   ✅ 能做的:
    --#      · 游戏**本身免费 / 无限次**的道具, 直接可用(本来就不消耗);
    --#      · **自动使用 / 自动装备** —— 省手速省操作, 消耗照旧;
    --#      · 把可用通道**尽量找全**(别名表), 提高命中"免费通道"的概率。
    --##########################################################################
    ...
    -- 一键把 每日/在线/新手/赛季 这类能领的都试一遍（别名表里十来个通道挨个来）
    ...
    -- 隔空获取。mode = "all" | "one"；one 时取"离我最近的"
    ...
    function SYS.ScanAll(opts)
        ...
        -- ★ v7.7.0: 统一扫描里顺带把反指纹自检也跑一遍(只读, 不耗时)
        ...
        -- 自检: 收集到的行数 vs 最终文本的行数(确认没有在拼接时丢东西)
        local expected = #L
        ...
        -- 文件（复用 6.9.0 的命名: 带游戏名 + PlaceId + 时间）
        local fn = nil
...
do
    ...
    -- 换服/重载地图后手动丢弃缓存(目前没人调, 留给以后接 TeleportService 的场合)
    ...
    function SYS.ESPTick()
        ...
        --   「🪧 头顶标签 (ESPNameTag)」一起走。这里做一次单向归一(每帧一个布尔判断, 可忽略),
        --   避免出现"开关显示关、头顶却还挂着武器标签"的不一致。关的时候两个一起关, 不会反复触发。
...
do
    ...
    local function tracerOrigin()
        -- ★★ v6.9.30 按用户要求改成【人物模型的第一视角射线】:
        --   起点 = 角色【头部/眼睛】(人身上), 方向 = 相机视线(准星指向)。
        --   原来是从【相机】出发 —— 第三人称下相机在角色身后, 画出来的线像是"凭空从背后射出";
        --   现在线一定从人身上出去, 方向仍跟着你看的地方(第一人称时头≈相机, 完全一致)。
        --   相机位置只在"角色还没加载出来"时当兜底。
    ...
    --##########################################################################
    local TPL={}                      -- ★ 强化: 多目标池(键=玩家, 值=Neon 薄片)
    ...
    local function tracerDraw(key,o,endP)
        ...
        if not q then
            local ok,pt=P(function()
                ...
                w.CanQuery=false w.CanTouch=false        -- ★ 必须: 别让射线/触碰打到自己这条线上
    ...
    function SYS.TracerTick()
        ...
        -- ★ 两种模式互斥: 切到"全部玩家"就收掉单目标那条; 切回单目标就清空池
        if all then
            if line then P(function() line:Destroy() end) line=nil end
        ...
        -- 单目标模式(原行为): 有锁定目标就画到目标身上; 没有就沿角色朝向画一条固定长度
            ...
            for _,t in ipairs(list:GetChildren()) do
                if isEntityTool(t) then
                    if not isExclusiveTool(t) then -- ★ 跳过独家物品
                        -- ★★ v8.6.0 改判【基础 CPS】(与背包显示一致), 不再用含等级/词缀的当前值
    ...
    function SYS.scanLowCPSCount(threshold)
        ...
        local function scan(list)
            ...
            for _,t in ipairs(list:GetChildren()) do
                if isEntityTool(t) then
                    if not isExclusiveTool(t) then -- ★ 跳过独家物品
                        -- ★★ v8.6.0 同 pickLow: 门槛按【基础 CPS】判(背包显示的那个值)
                        local cps=SYS.GetBrainrotBaseCPS(t)
...
do
    ...
    --##########################################################################
    --# ★★ v9.2.0 新增 · 服务端播报监听（用户要求「补强直接用」）
    --#   来源: 2026-09-21 综合扫描实锤 —— 这个游戏(踢一个幸运方块)的网络包
    --#   `ReplicatedStorage.Shared.Packages.Network` 上有 42 个 rev_ / 7 个 ref_,
    --#   其中【一批是我们以前完全没接的】: 倍率播报 / Boss 事件 / 糖果活动 / 天气 /
    --#   服务端消息 / Token / 被冰冻 / 好友。
    --#   ★ 这些【全是纯监听 OnClientEvent】—— 不改游戏状态、不点任何按钮、不发任何请求,
    --#     零副作用, 所以默认开着没风险。只做两件事: 控制台记一行 + 关键事件弹 HUD。
    --#   ⚠ 反过来: 扫描里那些【要主动触发】的通道(B_SellAll / AutoKickPurchase / candyManSkip…)
    --#     没有实机验证过参数, **本版一个都没接** —— 盲目调用可能造成不可逆的游戏内后果。
    --#     需要的话按需一个个试, 见 CHANGELOG 的「新功能候选」表。
    --##########################################################################
    local WATCH={
    ...
    SYS.StartServerWatch()          -- ★ v9.2.0 立即挂上(纯监听, 零副作用)
...
-- ★ 硅基流动(云端 OpenAI 兼容【限免】API)—— 翻译后端开关 SYS.T_.TransSili 打开时走它。
--   模型用 tencent/Hunyuan-MT-7B: 与本地 Hy-MT(混元翻译)同源, 且限免(不用付费的 Qwen)。
...
local SILI_MAXTOK=512      -- 云端没有 /props 可探, 输出上限用固定值(够用)
local SILI_MAXCONC=4       -- 免费版并发保守点, 避免撞限流

-- ★ 采样参数(本地和云端【完全一致】—— 确保切后端翻译结果不漂移, "一个模型"行为统一)。
...
local function unmask(s,tok,n)
    ...
    -- 模型只丢了一侧哨兵(写成 "7▮")时上面排不上 -> 会漏出一个孤立编号; 这两条把它清掉
...
-- 当前翻译后端: 本地 llama.cpp 或 硅基流动云端(SYS.T_.TransSili=true 时走云端)。
--   返回 url, key, model, maxTok(该后端输出上限), samp(该后端采样参数, 字段名已按后端定好)。
--   ★ 采样值(温度/核/候选)本地云端【完全一致】—— 切后端翻译结果不漂移, "一个模型"行为统一;
--     只有"重复惩罚"字段名不同(llama.cpp 叫 repeat_penalty, OpenAI/SiliconFlow 叫 repetition_penalty),
--     值都是 SAMP_REP_PEN。nil 字段会被 JSONEncode 自动省略, 所以两个后端各认各的名字。
...
local WaitQ={}         -- key -> {text=,prio=,cbs={}}(排队)
...
function Trans.checkLocal()
    -- 云端后端没有本地 /health 可探, 直接视为在线(失败退避 + 全局闸门已在请求层兜底)
...
function Trans.probeServer()
    if SYS.T_.TransSili==true then
        -- 云端没有 /props, 用固定上限(7B 有 32K 上下文, 输出 512; 并发保守 4 避免撞免费限流)
        Trans.SlotCtx=32768
    ...
    -- 在途上限取服务器真实槽位数(留 2 个余量给用户手动触发的强制重扫)
...
do
    ...
    -- 安装"踢人/抹除"拦截
    -- 安装"踢人/抹除"拦截
    -- ★★ v6.9.10 重写(与「隐藏 GUI」同一个病根: 性能)
    --   原来这里在 __namecall(所有实例方法调用的总入口) 上又挂了一条回调, 里面还做
    --   string.lower + find("kick") / find("ban") 子串搜索 —— 游戏每秒几百次方法调用全都要跑这一套,
    --   这是"开了过检测极度卡顿掉帧"的另一半原因。
    --   现在只留【单点 hookfunction】: 直接替换 LocalPlayer.Kick / LocalPlayer.Destroy ——
    --   只有游戏真要踢你的那一刻才走我们的空函数, 平时零开销。
    --   ⚠ 代价: 拦不住走别的通道的踢人(例如 game:Shutdown); 那种极少见,
    --   不值得用"全局每次调用都掉帧"去换。
    function Prot.InstallKickGuard()
        ...
        local ok,err=pcall(function()
            ...
            -- ★★ v6.9.23 补强(反踢): 再拦 game:Shutdown()(本地关服/退服) —— 单点 hookfunction, 平时零开销
            ...
            -- ★★ 补强(2026-09-20, 参考公开脚本的反踢手法): 再拦一个 "封禁入口" —— 新版 Roblox 的
            --   Players:BanAsync。同样是【单点 hookfunction】, 平时零开销 ——
            --   绝不去挂 __namecall 做关键词搜索, 那正是上面注释里"开了过检测极度卡顿掉帧"的病根。
            --   ⚠ 诚实边界: 这只是【本地拦一下】。真封禁是服务端权威的, 客户端拦不住。
            ...
            -- ★★ v6.9.23 补强(诊断): 被移出 Players 时打印一行(服务端断开拦不住, 但能立刻告诉你"被移走了")
    ...
    function Prot.RemoveKickGuard()
        ...
        if c.hf then
            ...
            -- ★★ 2026-09-20: BanAsync 也要还原(与上面三条对称)
    ...
    function Prot.InstallTPGuard()
        ...
        local ok,err=pcall(function()
            ...
            for i=1,#names do
                ...
                local f=TS[n]
                if type(f)=="function" then
                    -- ★ v6.8.1 同上: `old` 必须【先声明】再在闭包里引用, 否则拿到的是全局 old=nil
                    local old
                    local ok2=pcall(function()
            ...
            h=hookmetamethod(game,"__namecall",newcclosure(function(self,...)
                ...
                if m then
                    ...
                    if lm=="Raycast" then
                        ...
                        local a1,a2=...          -- ★ v6.9.18: 留下 origin/direction 用来补 Ray 字段
                        local res=h(self,...)
                        if SYS.T_.CB_SilentAim==true or SYS.T_.CB_BulletWall==true then
                            local tp=Ray.Target()
                            if tp then
                                Ray.Rewrites=Ray.Rewrites+1
                                -- ★ v6.9.0 增强: 补齐 RaycastResult 的全部字段。
                                --   原来只给 Instance/Position/Normal —— 游戏若读 Material 或
                                --   Distance 会拿到 nil(轻则判定异常, 重则露馅)。现在:
                                --   法线/材质尽量沿用"真实射线"的结果, 距离按真实起终点算。
                                ...
                                if not res then
                                    -- 真实射线打空了 -> 用"本地相机 -> 目标"估个距离
                                    local cam=WS.CurrentCamera
                                    if cam then dist=(cam.CFrame.Position-pos).Magnitude end
                                end
                                -- ★★ v6.9.18 补齐 RaycastResult 的【Ray】字段(枪械逻辑常读它):
                                --   真实射线命中时直接沿用它的 Ray; 打空了自己按 origin/direction 造一个。
                    ...
                    elseif lm=="FindPartOnRay" or lm=="FindPartOnRayWithIgnoreList"
                        ...
                        if SYS.T_.CB_SilentAim==true or SYS.T_.CB_BulletWall==true then
                            ...
                            if tp then
                                Ray.Rewrites=Ray.Rewrites+1
                                -- ★ v6.9.0 增强: 标准签名是 (Instance, Position, Normal, Material),
                                --   原来只返回 2 个 —— 补齐后 4 个都自洽。
    ...
    --============================================================
    -- ★★ v6.9.1 修(致命): 这里原来是 `local PG={} SYS.PG=PG` ——
    --   而 SYS.PG 在文件开头的初始化里已经被赋成【PlayerGui】, 这一行把它**整块覆盖成整蛊表**。
    --   后果(真机同样会中):
    --     · SYS.SafeParentGui 里的 gui.Parent=SYS.PG 挂到了一张普通表上 -> 菜单根本不显示;
    --     · 界面翻译的扫描根 scanRoot(SYS.PG)(Trans.forceRescan / 扫描循环)变成空表 -> UI 翻译扫不到任何控件;
    --     · 浮动按钮/F3 调试面板/「删广告 GUI」的父级与遍历根全部失效。
    --   WindUI 那条路径自己建 ScreenGui、不读 SYS.PG, 所以这个 bug 一直没暴露;
    --   回退旧自绘壳之后它立刻变成"菜单打不开"。整蛊模块改名 SYS.Prank, 不再占用 SYS.PG。
...
--   ①打得赢(战斗/视觉/移动) ②找得到人(玩家/传送/MachineParty) ③自动化(挂机) ④工具箱(功能/整蛊) ⑤辅助(翻译/设置)
...
UI.Pages["挂机"]=function(p)
    ...
    -- ★★ v8.2.0 新增: 「等级乘数」可调。原因 —— 旧代码把 CPS 算成
    --   `基础值 × 词缀 × 1.25^(等级-1)`, 那个 1.25 是**硬编码**的。
    --   如果本游戏的实际 CPS 不随等级指数增长, 这个乘数会把低 CPS 物品**算成高 CPS**,
    --   于是"门槛看起来没生效 / 什么都不卖"。默认 1.25(与改前逐字节一致), 设 1 = 不考虑等级。
...
UI.Pages["翻译"]=function(p)
    ...
    -- ★ v120.1 翻译后端: 本地 llama.cpp 或 硅基流动云端(免费 API), 二选一
...
--  ══════════════════════════════════════════════════════════════
    ...
    --##########################################################################
    local SCAN_CAP=80000            -- 全图遍历上限(大地图不卡死; 达上限会明说)
    ...
    -- 全图遍历(带上限)。I 层与 J 层共用 —— 这是"融合成一个功能"的落点。
    ...
    function LAB.FullScan()
        local t0=os.clock()
        SYS.ScanBuf={}                                  -- ★★ v9.2.0 清空落盘缓冲(本次扫描从头收)
        local _banner={"","##################  🔍 综合扫描  ##################",
...
verTag.TextColor3=CY.accent
    ...
    function SYS.CenterMenu()
        ...
        -- ★ v7.6.0: 手动回正中 = 同时【忘掉】触摸端记住的位置(否则下次打开又被拉回旧位置)
    ...
    function SYS.DiagUI()
        ...
        -- ★ v7.6.0: 触摸端适配 / 位置记忆 / 当前缩放 / 有没有把手, 一次看全
        ...
        -- ★ v7.7.0: 防护 / 移速护栏 / 反指纹 三个新面孔的状态, 一次看全
