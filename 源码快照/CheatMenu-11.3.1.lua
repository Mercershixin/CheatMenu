...
local SYS={
    ...
    T_={
        ...
        Prot_AntiAdmin=true,   -- ⛔ v10.4.0 删除: Prot_HideGui(「一键开启全部防护」的开关键, 用户说不需要了)
    ...
    C_={
        ...
        ForceCam="off",
        AutoTrainSec=5,   -- ⛔ v10.3.0 删除: RebirthCheck(自动重生检查间隔)
        ...
        DodgeDist=15,           -- 触发距离(格): 陷阱进到这个范围才开始退
        DodgeDepth=2,           -- 扫描深度: 1=只顶层 2=两层(原行为) 3=三层 4=四层
        DodgeCap=80,            -- 每层每个节点最多往下看几个子对象(防大图爆掉)
        DodgeScanSec=0.6,       -- 多久重新扫一遍陷阱列表(秒; 原来写死 2 秒)
        DodgeStep=0.35,         -- 退避速度(格/帧, 0.35≈21 格/秒; 上限 1.2)
...
SYS.N={
    ...
    Info   = ({"Status","Info","Overlay"})[math.random(1,3)],        -- ★ v10.2.0 战况面板也走中性名(与游戏自带 HUD 无法区分)
    F3     = "Stats",
    Combat = "Render",
    ...
    function SYS.SaveConfig()
        ...
        P(function()
            ...
            if withT then
                ...
                data._resumeT = os.time()   -- ★ v11.3.1: 记时间戳(原来只记 true, 无法判时效)
            end
...
do
    ...
    local RRemoteCache={}
    local NEGC={}                    -- ★ v9.10.0 全域兜底的【负缓存】: 扫过没找到的, 8 秒内不再重扫
...
do
    ...
    function SYS.RemoteRisk(name)
        ...
        local lf=flat(low)          -- ★ 去分隔符版本, 词组一律用它比对(见 flat 的说明)
    ...
    function SYS.IsHoneypot(obj)
        ...
        for _=1,4 do
            ...
            if type(nm)=="string" and nm~="" then
                local low=nm:lower()
                local lf=flat(low)      -- ★ 同 RemoteRisk: camelCase `DroneStickyNote` 也要能命中词组
...
do
    ...
    function SYS.ValOf(inst,cls,name)
        ...
        local ok2,is=pcall(function() return c:IsA(cls) end)      -- 只认类名对得上的(值对象层)
    ...
    function IP.Tick()
        ...
        IP.Next=now+0.4                       -- 2.5 次/秒: 状态类信息不需要每帧刷
        local ok,L=P(IP.Snapshot)
        if not ok or type(L)~="table" then return end
        if #L==0 then
            if IP.Gui then IP.Clear() end      -- 没有任何可显示的状态 -> 整块隐藏(不留假面板)
            return
        end
        ...
        P(function()
            ...
            local want=14+nl*18               -- 行数变了就跟着长高(不写死高度, 免得最后一行被截)
    ...
    function SP.Hook()
        ...
        for nm,f in pairs(SP.Found) do
            local orig                              -- ★ 必须先声明: 闭包才把它当 upvalue 捕获
            local body=function(...)
                ...
                if type(orig)~="function" then return end   -- 兜底(理论上探针已挡住)
                return orig(...)                            -- ★ 原样转发: 不改返回值/入参
            end
    ...
    function SP.Tick()
        ...
        SP.Next=now+0.2                      -- 5 次/秒: 计时类信息不需要每帧刷
        local ok,model,hd=P(SP.FindSpider)
        if not ok or model==nil or hd==nil then
            if SP.Gui then SP.Clear() end     -- 场上没有蜘蛛 -> 整块隐藏(不留假标签)
            return
        end
        ...
        P(function()
            if not (SP.Gui and SP.Gui.Parent) then
                ...
                g.Name=SYS.N.Spider                 -- 中性名(反指纹, 与 SYS.NewVis 同一口径)
                ...
                t.TextColor3=Color3.fromRGB(255,130,60)   -- ★ 怪物·NPC 橙系, 与配色规范一致
                t.TextScaled=true
    ...
    function SYS.SetSpiderSense(on)
        ...
        if on then
            ...
            SP.Clear()
            P(SP.Unhook)      -- ★ 关掉就把被替换的函数还原 —— 不留 hook 在游戏里空转
    ...
    DO.FREEZE={"redraw","writeBar"}                    -- ① 冻住: 这两条一停, 条/数字就停在最后一帧
    DO.NOBLOW={"blowDynamite"}                         -- ② 不炸: 断掉致死的那一步
    DO.WATCH ={"igniteFuse","perfectPress","tryPress"} -- 只观测(供 HUD 说实话, 也为下一版取证)
    ...
    function DO.Scan()
        ...
        local want={}
        local want_list={}   -- ★★ v11.0.0 (B1): 指纹回退要按名字逐个查, 需要一份有序名单
    ...
    local function doWant()
        ...
        if next(w)~=nil then                       -- 有任何一档开着, 才挂观测
    ...
    local function doHookOne(nm,f)
        ...
        local body=function(...)
            ...
            if type(orig)~="function" then return nil end   -- 兜底(探针已挡住)
            return orig(...)                                -- ★ 原样转发: 不改返回值/入参
        end
        local ok,o=pcall(HF,f,NCC and NCC(body) or body)
        if ok and type(o)=="function" then
    ...
    function DO.Tick()
        ...
        DO.Next=now+0.2                      -- 5 次/秒: 状态类信息不需要每帧刷
        ...
        P(function()
            if not (DO.Gui and DO.Gui.Parent) then
                ...
                g.Name=SYS.N.Info                 -- 中性名(与战况面板同一档: 与游戏自带 HUD 无法区分)
    ...
    function SYS.SetDeadOn(kind,on)
        ...
        if both then
            P(DO.Sync)                  -- ★ 只动这一档要的函数, 另一档继续有效
            DO.Next=0
            SYS.SetLoop("DeadOn",true,RS.Heartbeat,DO.Tick)
            P(DO.Tick)
        else
            DO.UnhookAll()              -- ★ 两档全关 = 把游戏自己的函数【全部还原】, 不留 hook
            DO.Clear()
    ...
    function DR.Scan()
        ...
        local fpcnt=0
        for i=1,#list do
            if os.clock()-t0>4 then break end          -- ★ 时间预算: 别把加载卡住
            local v=list[i]
            if type(v)=="function" then
                local sn=drScript(v)
    ...
    --##########################################################################
    SYS.GC_TTL = 20          -- GC 快照缓存时长(秒): 同一份表 20 秒内复用, 到期自动重取
...
do
    ...
    --#     所以这是个"加形态"而不是"换掉原来的"。
    --##########################################################################
    local HL_SOLID = 0.35      -- 「看得见」时的填充透明度(越小越实心)
    local HL_LINE  = 0.93      -- 「在墙后」时的填充透明度(近全透 = 只剩描边)
    local function espFill(h, wall)
        if not h then return end
        if SYS.T_.ESP_WallWise == false then
            h.FillTransparency = wall and 0.93 or 0.88   -- 关掉 = 逐字恢复 v10.6.0 的旧观感
        else
    ...
    --##########################################################################
    SYS.DRHostileMount = {Horse=true, War=true}                -- EntityName 里的坐骑(绝不标红)
    SYS.DRHostileRoots = {"RuntimeEntities","NightEnemies"}    -- 敌人所在容器的名字(Workspace 下)
    ...
    function SYS.ESPTick()
        ...
        if SYS.T_.ESP then
            ...
            for p,c in pairs(act) do
                ...
                if not h then
                    ...
                    espFill(h,false)                                  -- ★ v10.7.0: 本循环下面每帧会按遮挡重算
                    h.Parent=c
                    HL[p]=h
        ...
        if SYS.T_.ESP_NPC then
            ...
            for m in pairs(nact) do
                ...
                espFill(h,wall)                          -- ★ v10.7.0: 隔墙=只留描边 / 可见=全身实心
        ...
        tl.Text=(p.DisplayName or p.Name).."  "..hp.."/"..mx.._ext.._dist
                        if _dead then tl.TextColor3=Color3.fromRGB(140,145,155)   -- ★ v10.2.0: 权威状态=死 -> 灰掉
        ...
        if SYS.T_.ESPItem then
            ...
            for o in pairs(actI) do
                ...
                if not h then
                    ...
                    h.Adornee=o
                    h.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop
                    espFill(h,false)                         -- ★ v10.7.0: 下面的每帧循环会按遮挡重算
                    h.OutlineTransparency=0
    ...
    --##########################################################################
    local HITMK_MAX = 8          -- 同时可见小牌上限(超了复用最早那块, 不无限堆实例)
    local HITMK_SEC = 1.2        -- 每块小牌存活秒数
    SYS._hitMk      = SYS._hitMk      or {}   -- 牌子池(按序号复用)
    SYS._hitMkAt    = SYS._hitMkAt    or {}   -- 每块牌子的过期时刻
    SYS._hitMkIdx   = SYS._hitMkIdx   or 0
    SYS._hitMkCount = 0                       -- 本局累计自动触发次数(开关重开清零)
    ...
    function SYS.AutoHitTick()
        ...
        SYS.HitMarkTick()   -- ★★ v10.5.3: 小牌 / 屏幕行的过期回收
    end
...
do
    ...
    local AFK={Kick=nil,LastBonus=0,BonusWin=0,WTool=nil,WToolT=0}   -- ⛔ v10.3.0 去掉 Reb(自动重生已删)
    SYS.AFK=AFK
    task.spawn(function()
        task.wait(2)
...
SYS.MIGOLDDEFAULTS={          -- 旧默认值: 用来判断"这一项用户有没有手动调过"
...
function SYS.MaxParams(force)
    ...
    P(SYS.QueueSave)          -- 存盘, 否则下次加载又回到旧值
    return n
end
...
local HOOK_MAX=6000       -- 文本变更钩子数量上限
local HOOK_PER_SEC=200    -- 每秒最多新挂 200 个(摊平首次扫描的突发)
...
local function onTextChanged(obj)
    ...
    if last and now-last<RETRY_GAP then return end        -- 0.1s 内的重复变更合并
    TextLastAt[obj]=now
    ...
    if not w or now-w.t>RETRY_WIN then
        ...
        w.n=w.n+1
        if w.n>RETRY_WIN_N then return end     -- 这一窗口内太频繁才放弃, 下一窗口自动重置
    end
    task.defer(function()
        if Trans.Unloaded or not Trans.UIScanActive then return end
...
do
    ...
    function ST.Harvest()
        ...
        markDict(SYS._Hooks,true)                                  -- 统一 SafeHook 挂的(键=目标函数)
        ...
        if SYS.DRCombat then
            markDict(SYS.DRCombat.Found)                           -- 列表: 值=被挂函数
        ...
        if SYS.RayHook then markDict(SYS.RayHook.FnHooked,true) end -- 键=被挂函数
        if SYS.Gun and type(SYS.Gun.Nums)=="table" then            -- 数值槽里 e.f = 被挂函数
    ...
    function SYS.SetStealth(on)
        if on then
            ...
            local got=ST.Harvest()                     -- ★ 把现有全部功能的 hook 目标也收进来
    ...
    function Ray.Through(origin,dir)
        ...
        Ray.Busy=true                 -- 挡住我们自己的 __namecall hook, 别把这条也扭了
                            ...
                            if tp then
                                ...
                                if typeof(o)=="Vector3" and typeof(dv)=="Vector3"
                                   and dv.Magnitude>0.001 and RayCtor and RayCtor.new then
                                    rayObj=RayCtor.new(o,dv)     -- 原样重建: 方向/长度与游戏当初传的一致
                                end
    ...
    --==========================================================================
    local function RAPI(n)                    -- 执行器 API 一律走 _G 查表(不裸写名字)
        local ok,v=pcall(function() return _G[n] end)
        if ok and type(v)=="function" then return v end
        return nil
    end
    ...
    do
        local G={} SYS.Gun=G
        local Cands={recoil={}}                 -- 扫描候选(名字带 recoil 的函数)
        local maxNums=90                        -- 数值槽上限(防"写几百个 upvalue"把游戏改花)
        G.Cands=Cands
        ...
        local function UVAL(f,i)                  -- 读 upvalue 的【值】
        ...
        local function SUV(f,i,v)                 -- 写 upvalue
            local g=(DBG and DBG.setupvalue) or APIfn("setupvalue")
            if not g then return false end
            return (pcall(g,f,i,v))
        end
        local function upNames(f)                 -- 一次读回全部 upvalue 名(最多 24 个)
            local g=(DBG and DBG.getupvalue) or APIfn("getupvalue")
            if not g then return nil end
            local t,n={},1
            while n<=24 do
...
function SYS.UnloadAll()
    ...
    P(function() if SYS.Info then SYS.Info.Clear() end end)   -- ★ v10.2.0 战况面板: 卸载对称(建了什么清什么)
    ...
    P(SYS.StopTrain) P(SYS.StopGym)   -- ⛔ v10.3.0: StopReb 随「自动重生」删除
