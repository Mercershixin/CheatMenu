...
| ★ `2026-09-23_绕过能力补强_开源技法测绘与候选.md` | **跨项目专题**（本脚本自身的能力加固） | 开源绕过技法全测绘 **T1~T15**（结构指纹定位 / 元表剥离 / debug.info 伪装 / setupvalue 原地换 / raknet 包级拦截 / CanTouch） + 本项目对照（已有/缺口/红线禁） + 落地候选 **B1~B7** | — |

> ★ 最后两行是**专题文档**（不属某个游戏的 Remote 清单）：起因是用户问"防攻击"与"绕过能力补强"。
